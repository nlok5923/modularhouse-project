...
impl AlgoRunner {
    ...
    pub fn run_bfv_clob_algo(self, contents: String) {
        ...
        for (index, order) in encrypted_buy_orders.iter().enumerate() {
            ...
            match is_less_plain[0] {
                0 => {
                    ...
                    let zero_value_order_encoded =
                        evaluator.plaintext_encode(&zero_value_order, Encoding::default());
                    let zero_value_order_encrypted =
                        evaluator.encrypt(&sk, &zero_value_order_encoded, &mut rng);
        ...
        let buy_orders_filled_plain = buy_orders_filling_encrypted
            .iter()
            .map(|x| {
                evaluator.plaintext_decode(&evaluator.decrypt(&sk, &x), Encoding::default())[0]
            })
            .collect::<Vec<u64>>();
